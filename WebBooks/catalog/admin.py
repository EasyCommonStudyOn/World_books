...
class AuthorAdmin(admin.ModelAdmin):
    list_display = ('last_name', 'first_name', 'date_of_birth',
                    'photo')  # Чтобы показать большее число полей, изменить порядок их отображения или ввести дополнительные поля, можно создать новый кортеж list_display.


admin.site.register(Author, AuthorAdmin)


@admin.register(Book)  # для создания и регистрации новых моделей мы воспользовались декоратором @register ( он делает то же самое, что и метод admin.site.register()):
class BookAdmin(admin.ModelAdmin):
    ...
    """
    list_filter = ('genre', 'authors') #Здесь к классу, описывающему книги, к списку отображаемых полей добавлен фильтрна жанр и на авторов книг
